...
//
// Same reason run-profiles.mjs through run-usage.mjs exist: the driver writes its
...
if (
  !reportAudit(
    'settings-check',
    auditReport({ driver: 'settingscheck.ts', checks, only: auditOnly?.slice('--only='.length) })
  )
) {
  process.exit(1)
}
